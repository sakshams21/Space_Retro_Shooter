		...
		while (true) {
		  float waveType = Random.Range(0.0f, 10.0f);
			for (int i = 0 ; i < enemiesPerWave; i++) {
				Vector3 topLeft = cameraRef.ScreenToWorldPoint(new Vector3(0, cameraRef.pixelHeight + 2, 0));
				Vector3 topRight = cameraRef.ScreenToWorldPoint(new Vector3(cameraRef.pixelWidth, cameraRef.pixelHeight + 2, 0));
